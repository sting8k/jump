...
- The shipped themes are `default`, `spacetime`, `vercel`, `hud`, and `slate-noir`; `default` keeps the existing Web UI visuals.
...
- API: add `PATCH /v1/frontend-preferences` with `{ "appearance": { "theme_id": "vercel" } }` or any other whitelisted theme id such as `hud` or `slate-noir`.
