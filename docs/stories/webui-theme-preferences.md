...
The Web UI has client-side theme switching for chrome/UI surfaces. It ships the existing `default` theme, a `spacetime` theme inspired by SpacetimeDB's dark blue-black surfaces, a `vercel` theme with a Command Center direction using black cockpit surfaces, HUD-cyan state signals, tactical grid texture, selected-session lock-on treatment, and instrument-style telemetry, a `hud` theme with a Signal HUD direction using blue-gray instrument glass, muted green/teal status accents, subtle grid texture, and angular telemetry frames, a `slate-noir` theme with a Slate Noir Premium Workspace direction using warm matte obsidian surfaces, warm off-white text, titanium-gray secondary text, hairline steel borders, soft emerald activity, and orange warning highlights, and a `zerobyte` theme inspired by Zerobyte's graphite documentation UI using near-black panels, neutral document text, hard edges, and a restrained ember-orange `#ff543a` action/status accent. The selected `theme_id` is applied immediately in the browser, cached locally for first paint, and persisted by `jumpd` as server-managed Web UI state without page reloads or terminal/session reconnects.
...
- The shipped themes are `default`, `spacetime`, `vercel`, `hud`, `slate-noir`, and `zerobyte`; `default` keeps the existing Web UI visuals.
...
- API: add `PATCH /v1/frontend-preferences` with `{ "appearance": { "theme_id": "vercel" } }` or any other whitelisted theme id such as `hud`, `slate-noir`, or `zerobyte`.
...
- `corepack pnpm --filter @jump/web test -- appearance.test.ts store.test.ts --runInBand` passed (Vitest ran the full `@jump/web` suite: 22 files, 366 tests).
